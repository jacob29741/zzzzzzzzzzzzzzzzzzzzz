...
async def auto_self_check():
    ...
    #자가진단 실행 if문
    if datetime.datetime.now().hour == 1 and datetime.datetime.now().minute == start_minute and last_day != datetime.datetime.now().strftime('%Y-%m-%d') and datetime.datetime.today().weekday()<5:
    #if True:
        print("실행")
    ...
    if datetime.datetime.now().hour == 6 and datetime.datetime.now().minute == 0:
        ...
        for user_id in user_data.keys():
            ...
            try:
                ...
                user = await bot.fetch_user(525295194677968901)
...
async def on_ready():  
    ...
    with open(json_file_name, "r",encoding='utf-8-sig') as json_file:
        user_data=json.load(json_file)
    game = discord.Game(f" ㅈ명령어 | {len(user_data.keys())}명의 자가진단을 처리 중 ")
        ...
        if erorr != "erorr":
            if data["code"]=="SUCCESS":
                ...
                #학년반정보가 등록되지 않아서 시간표 정보 출력이 불가능 할 경우
                    ...
                    embed.add_field(name = "오늘의 시간표",value = f"학년반 정보가 없습니다. `ㅈ학년반정보`으로 입력해주십시오.")
                #급식 정보 전송
    ...
    if str(ctx.guild) == "None":
        if name!=None and birth!=None and area!=None and school_name!=None and school_type!=None and passward!=None:
            if area not in back_area_list:
                ...
                embed.add_field(name="입력 형식",value=f"ㅈ정보등록 [이름] [생년월일] [지역] [학교이름] [학교타입] [비밀번호]{end_msg}", inline=False)
...
async def 관리자정보확인(ctx):
    if str(ctx.author.id) == '525295194677968901':
...
@bot.command()
async def 개인공지(ctx,name,birth,*,msg):
    if str(ctx.author.id) == '525295194677968901':
...
async def 급식(ctx, day=None,user: discord.User=None):
    ...
    if len(day) == 8 and day.isdigit():
        ...
        if user in user_data:
            ...
            await ctx.send("해당 유저의 데이터가 없습니다. `ㅈ정보등록`으로 유저 데이터를 입력해주십시오.")
    else:
...
@bot.command()
async def 학년반정보(ctx, school_grade:str=None, school_class:str=None):
    if school_grade == None or school_class == None:
        await ctx.send("`ㅈ학년반정보 [학년] [반]`의 형식으로 입력해주십시오. (예 : `ㅈ학년반정보 1 3`)")
    else:
    ...
    if len(day) == 8 and day.isdigit():
        ...
        if user in user_data:
            ...
            if "school_grade" not in user_data_keys and "school_class" not in user_data_keys:
                await ctx.send("입력된 학년반 데이터가 없습니다. `ㅈ학년반정보` 으로 입력해주십시오.")
            else:
                if "school_code" not in user_data_keys and "area_code" not in user_data_keys:
                    user_data[user]["area_code"] = await get_school_data.get_area_code(user_data[user]["area"])
                    user_data[user]["school_code"] = await get_school_data.get_school_code(user_data[user]["school_name"],user_data[user]["area_code"])
...
@bot.command()
async def 자가진단(ctx):
    await ctx.send(f"수동자가진단은 `{config['prefix']}진단참여`을 이용해주시기 바랍니다.")
...
async def 진단참여(ctx):
    ...
    if user in user_data.keys():
        ...
        await ctx.send("유저 데이터에 등록된 정보가 없습니다. `ㅈ정보등록`으로 등록해주십시오.")
